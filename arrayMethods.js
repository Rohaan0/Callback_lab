...
// CODE HERE
const postTaxPrices = prices.map(num => num * 1.07) // = prices.map(/* Provide Your Callback Here );
...
newPrice = orders.map(orderObj => orderObj.price + orderObj.price * orderObj.tax)
...
// CODE HERE
const filteredHighOrder filteredHighOrder.map(objNum => objNum.price)
const fullHighOrder = mappedHighOrder.reduce((acc, curr) => acc + curr)
console.log(fullHighOrder)